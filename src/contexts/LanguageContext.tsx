import React, { createContext, useContext, ReactNode } from "react";
import { useLocalStorage } from "@/hooks/useLocalStorage";

type Language = "ko" | "en" | "ja";

interface LanguageContextType {
  language: Language;
  setLanguage: (lang: Language) => void;
  t: (key: string) => string;
}

const translations: Record<Language, Record<string, string>> = {
  ko: {
    "header.title": "뮤지컬 대본 노트",
    "header.subtitle": "모두를 위한 뮤지컬 대본 창작 플랫폼",
    "basic.title": "작품 기본정보",
    "basic.workTitle": "작품명",
    "basic.workTitle.placeholder": "뮤지컬 제목을 입력하세요",
    "basic.timeSetting": "시간 배경",
    "basic.timeSetting.placeholder": "시간적 배경을 입력하세요",
    "basic.spaceSetting": "공간 배경",
    "basic.spaceSetting.placeholder": "공간적 배경을 입력하세요",
    "basic.synopsis": "시놉시스",
    "basic.synopsis.placeholder": "작품의 시놉시스를 입력하세요",
    "characters.title": "등장인물",
    "characters.add": "인물 추가",
    "characters.name": "이름",
    "characters.name.placeholder": "인물 이름",
    "characters.description": "설명",
    "characters.description.placeholder": "인물 설명",
    "characters.delete": "삭제",
    "characters.empty": "등장인물을 추가해주세요",
    "characters.count": "명",
    "characters.together": "다함께",
    "chapters.title": "장 구성",
    "chapters.add": "장 추가",
    "chapters.chapterLabel": "장",
    "chapters.chapterTitle": "장 제목",
    "chapters.chapterTitle.placeholder": "장 제목을 입력하세요",
    "chapters.delete": "장 삭제",
    "chapters.addBlock": "블록 추가",
    "chapters.narration": "해설",
    "chapters.dialogue": "대사",
    "chapters.song": "노래",
    "chapters.narration.placeholder": "해설 내용을 입력하세요",
    "chapters.dialogue.placeholder": "대사 내용을 입력하세요",
    "chapters.selectCharacter": "인물 선택",
    "chapters.songTitle": "노래 제목",
    "chapters.songTitle.placeholder": "노래 제목을 입력하세요",
    "chapters.addLyric": "가사 추가",
    "chapters.lyric.placeholder": "가사 내용을 입력하세요",
    "chapters.deleteBlock": "블록 삭제",
    "chapters.deleteLyric": "가사 삭제",
    "chapters.empty": "블록을 추가하여 대본을 작성하세요",
    "chapters.selectSinger": "가수 선택",
    "chapters.addDialogue": "대사 추가",
    "chapters.addSongNarration": "해설 추가",
    "chapters.songNarration.placeholder": "노래 중 해설 내용을 입력하세요",
    "blocks.dragTooltip": "드래그하여 순서 변경",
    "actions.close": "닫기",
    "actions.title": "내보내기",
    "actions.download.txt": "TXT 다운로드",
    "actions.download.docx": "DOCX 다운로드",
    "actions.download.pdf": "PDF 내보내기",
    "actions.download.json": "JSON 저장",
    "actions.upload.json": "JSON 불러오기",
    "actions.json.loaded": "데이터를 불러왔습니다!",
    "actions.json.error": "파일을 불러올 수 없습니다.",
    "actions.copy": "클립보드에 복사",
    "actions.fullView": "전체 보기",
    "actions.print": "인쇄",
    "actions.copied": "클립보드에 복사되었습니다!",
    "actions.shareLink": "링크 공유",
    "actions.shareLink.copied": "공유 링크가 클립보드에 복사되었습니다!",
    "actions.shareLink.loaded": "공유된 데이터를 불러왔습니다!",
    "reset.button": "데이터 초기화",
    "reset.confirm.title": "정말 초기화하시겠습니까?",
    "reset.confirm.message": "모든 작성 내용이 삭제됩니다. 이 작업은 되돌릴 수 없습니다.",
    "reset.confirm.yes": "초기화",
    "reset.confirm.no": "취소",
    "footer.text": "created by. 교육뮤지컬 꿈꾸는 치수쌤",
  },
  en: {
    "header.title": "Musical Script Notebook",
    "header.subtitle": "A Musical Script Creation Platform for Everyone",
    "basic.title": "Basic Information",
    "basic.workTitle": "Title",
    "basic.workTitle.placeholder": "Enter musical title",
    "basic.timeSetting": "Time Setting",
    "basic.timeSetting.placeholder": "Enter time setting",
    "basic.spaceSetting": "Space Setting",
    "basic.spaceSetting.placeholder": "Enter space setting",
    "basic.synopsis": "Synopsis",
    "basic.synopsis.placeholder": "Enter synopsis",
    "characters.title": "Characters",
    "characters.add": "Add Character",
    "characters.name": "Name",
    "characters.name.placeholder": "Character name",
    "characters.description": "Description",
    "characters.description.placeholder": "Character description",
    "characters.delete": "Delete",
    "characters.empty": "Please add characters",
    "characters.count": "",
    "characters.together": "All Together",
    "chapters.title": "Chapters",
    "chapters.add": "Add Chapter",
    "chapters.chapterLabel": "Scene",
    "chapters.chapterTitle": "Chapter Title",
    "chapters.chapterTitle.placeholder": "Enter chapter title",
    "chapters.delete": "Delete Chapter",
    "chapters.addBlock": "Add Block",
    "chapters.narration": "Narration",
    "chapters.dialogue": "Dialogue",
    "chapters.song": "Song",
    "chapters.narration.placeholder": "Enter narration",
    "chapters.dialogue.placeholder": "Enter dialogue",
    "chapters.selectCharacter": "Select Character",
    "chapters.songTitle": "Song Title",
    "chapters.songTitle.placeholder": "Enter song title",
    "chapters.addLyric": "Add Lyric",
    "chapters.lyric.placeholder": "Enter lyrics",
    "chapters.deleteBlock": "Delete Block",
    "chapters.deleteLyric": "Delete Lyric",
    "chapters.empty": "Add blocks to write your script",
    "chapters.selectSinger": "Select Singer",
    "chapters.addDialogue": "Add Dialogue",
    "chapters.addSongNarration": "Add Stage Direction",
    "chapters.songNarration.placeholder": "Enter stage direction within song",
    "blocks.dragTooltip": "Drag to reorder",
    "actions.close": "Close",
    "actions.title": "Export",
    "actions.download.txt": "Download TXT",
    "actions.download.docx": "Download DOCX",
    "actions.download.pdf": "Export PDF",
    "actions.download.json": "Save JSON",
    "actions.upload.json": "Load JSON",
    "actions.json.loaded": "Data loaded successfully!",
    "actions.json.error": "Failed to load file.",
    "actions.copy": "Copy to Clipboard",
    "actions.fullView": "Full View",
    "actions.print": "Print",
    "actions.copied": "Copied to clipboard!",
    "actions.shareLink": "Share Link",
    "actions.shareLink.copied": "Share link copied to clipboard!",
    "actions.shareLink.loaded": "Shared data loaded!",
    "reset.button": "Reset Data",
    "reset.confirm.title": "Are you sure?",
    "reset.confirm.message": "All data will be deleted. This cannot be undone.",
    "reset.confirm.yes": "Reset",
    "reset.confirm.no": "Cancel",
    "footer.text": "created by. 교육뮤지컬 꿈꾸는 치수쌤",
  },
  ja: {
    "header.title": "ミュージカル台本ノート",
    "header.subtitle": "みんなのためのミュージカル台本創作プラットフォーム",
    "basic.title": "作品基本情報",
    "basic.workTitle": "作品名",
    "basic.workTitle.placeholder": "ミュージカルのタイトルを入力してください",
    "basic.timeSetting": "時間設定",
    "basic.timeSetting.placeholder": "時間的背景を入力してください",
    "basic.spaceSetting": "空間設定",
    "basic.spaceSetting.placeholder": "空間的背景を入力してください",
    "basic.synopsis": "あらすじ",
    "basic.synopsis.placeholder": "作品のあらすじを入力してください",
    "characters.title": "登場人物",
    "characters.add": "人物追加",
    "characters.name": "名前",
    "characters.name.placeholder": "人物の名前",
    "characters.description": "説明",
    "characters.description.placeholder": "人物の説明",
    "characters.delete": "削除",
    "characters.empty": "登場人物を追加してください",
    "characters.count": "人",
    "characters.together": "みんなで",
    "chapters.title": "章の構成",
    "chapters.add": "章を追加",
    "chapters.chapterLabel": "幕",
    "chapters.chapterTitle": "章のタイトル",
    "chapters.chapterTitle.placeholder": "章のタイトルを入力してください",
    "chapters.delete": "章を削除",
    "chapters.addBlock": "ブロック追加",
    "chapters.narration": "ナレーション",
    "chapters.dialogue": "セリフ",
    "chapters.song": "歌",
    "chapters.narration.placeholder": "ナレーションの内容を入力してください",
    "chapters.dialogue.placeholder": "セリフの内容を入力してください",
    "chapters.selectCharacter": "人物を選択",
    "chapters.songTitle": "曲名",
    "chapters.songTitle.placeholder": "曲名を入力してください",
    "chapters.addLyric": "歌詞を追加",
    "chapters.lyric.placeholder": "歌詞の内容を入力してください",
    "chapters.deleteBlock": "ブロック削除",
    "chapters.deleteLyric": "歌詞削除",
    "chapters.empty": "ブロックを追加して台本を作成してください",
    "chapters.selectSinger": "歌手を選択",
    "chapters.addDialogue": "セリフを追加",
    "chapters.addSongNarration": "ト書きを追加",
    "chapters.songNarration.placeholder": "歌の中のト書きを入力してください",
    "blocks.dragTooltip": "ドラッグして並び替え",
    "actions.close": "閉じる",
    "actions.title": "エクスポート",
    "actions.download.txt": "TXTダウンロード",
    "actions.download.docx": "DOCXダウンロード",
    "actions.download.pdf": "PDFエクスポート",
    "actions.download.json": "JSON保存",
    "actions.upload.json": "JSON読み込み",
    "actions.json.loaded": "データを読み込みました！",
    "actions.json.error": "ファイルを読み込めません。",
    "actions.copy": "クリップボードにコピー",
    "actions.fullView": "全体表示",
    "actions.print": "印刷",
    "actions.copied": "クリップボードにコピーしました！",
    "actions.shareLink": "リンク共有",
    "actions.shareLink.copied": "共有リンクをコピーしました！",
    "actions.shareLink.loaded": "共有データを読み込みました！",
    "reset.button": "データ初期化",
    "reset.confirm.title": "本当に初期化しますか？",
    "reset.confirm.message": "すべてのデータが削除されます。この操作は元に戻せません。",
    "reset.confirm.yes": "初期化",
    "reset.confirm.no": "キャンセル",
    "footer.text": "created by. 교육뮤지컬 꿈꾸는 치수쌤",
  },
};

const LanguageContext = createContext<LanguageContextType | undefined>(undefined);

export function LanguageProvider({ children }: { children: ReactNode }) {
  const [language, setLanguage] = useLocalStorage<Language>("scriptnote-lang", "ko");

  const t = (key: string): string => {
    return translations[language][key] || key;
  };

  return (
    <LanguageContext.Provider value={{ language, setLanguage, t }}>
      {children}
    </LanguageContext.Provider>
  );
}

export function useLanguage() {
  const context = useContext(LanguageContext);
  if (!context) {
    throw new Error("useLanguage must be used within LanguageProvider");
  }
  return context;
}
