...
import { Download, Copy, ExternalLink, Printer, FileText, FileDown, Save, Upload } from "lucide-react";
import { exportToTxt, exportToDocx, exportToPdf, exportToJson, importFromJson, copyToClipboard, openFullView } from "@/utils/exportUtils";
import { toast } from "sonner";

interface ActionButtonsProps {
  scriptData: ScriptData;
  onImport?: (data: ScriptData) => void;
}

export function ActionButtons({ scriptData, onImport }: ActionButtonsProps) {
  const { t } = useLanguage();
  ...
  const actions = [
    ...
    { label: t("actions.download.json"), icon: Save, emoji: "💾", onClick: () => exportToJson(scriptData), color: "border-[hsl(140,50%,78%)] bg-[hsl(140,50%,96%)] hover:bg-[hsl(140,50%,92%)]" },
    { label: t("actions.upload.json"), icon: Upload, emoji: "📂", onClick: handleImport, color: "border-[hsl(30,60%,80%)] bg-[hsl(30,50%,96%)] hover:bg-[hsl(30,50%,92%)]" },
